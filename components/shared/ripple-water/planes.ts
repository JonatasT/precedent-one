...
export class Planes {
  ...
  uniforms: { uPlaneSize: THREE.Uniform<THREE.Vector2> };
  constructor(sceneManager: any, images: any) {
  ...
  load(loader: { begin: (arg0: string) => void; end: (arg0: string) => void }) {
  ...
  onResize(width: any, height: any) {
    ...
    this.meshes.forEach(
      (
        mesh: { geometry: { dispose: () => void }; position: { x: number } },
        i: number,
      ) => {
